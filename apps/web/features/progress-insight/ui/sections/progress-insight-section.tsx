...
import { CalendarRange, Flame, LineChart, LockKeyhole, Scale, Sparkles, TrendingUp } from 'lucide-react';
...
function getDirectionCopy(progress: ProgressRangeData): string {
  ...
  const changeText = change === null ? '--' : `${change > 0 ? '+' : ''}${change.toFixed(1)}kg`;
...
export function ProgressInsightSection() {
  ...
  if (isLoading || !progress || !report) {
    return <Skeleton className="h-[620px] w-full rounded-3xl" />;
  }

  const weightChange = formatWeightChange(progress.weightTrendPoints);
  const weightChangeText = weightChange === null ? '--' : `${weightChange > 0 ? '+' : ''}${weightChange.toFixed(1)} kg`;
    ...
    <Card className="w-full border-border/70 bg-card/92 backdrop-blur motion-enter">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <Button variant={range === 'weekly' ? 'default' : 'outline'} onClick={() => setRange('weekly')}>
            7 日
          </Button>
          <Button variant={range === 'monthly' ? 'default' : 'outline'} onClick={() => setRange('monthly')}>
            30 日
          </Button>
        </div>
            ...
            <div className="mt-4 grid gap-2">
              {progress.weightTrendPoints.slice(-7).map((point) => (
                <div key={point.date} className="flex items-center justify-between rounded-xl bg-muted/55 px-3 py-2 text-sm">
                  ...
                  <span className={point.isMissing ? 'text-muted-foreground' : 'font-medium text-foreground'}>
          ...
          <div className="grid gap-4">
            <div className="rounded-3xl border border-border/70 bg-muted/55 p-4">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Badge variant="outline" className="rounded-full">运动 2 / 3 / 5 次里程碑</Badge>
                <Badge variant="outline" className="rounded-full">恢复后 48 小时回归</Badge>
              </div>
            </div>
            <div className="rounded-3xl border border-border/70 bg-background/85 p-4">
              ...
              <p className="mt-3 text-sm text-muted-foreground">当前里程碑：{progress.milestone.current}</p>
              <p className="mt-1 text-sm text-muted-foreground">下一步：{progress.milestone.next}</p>
            </div>
          </div>
        </div>

        <div className="flex flex-wrap gap-3">
          <Link href="/dashboard" className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'rounded-xl')}>
            回首页继续今天
          </Link>
          <Link href="/checkins" className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'rounded-xl')}>
            去记录中心
          </Link>
        </div>
